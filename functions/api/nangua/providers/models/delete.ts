...
export const onRequestPost = async ({
  ...
  const body = await readJson<DeleteModelBody>(request);
  ...
  const nextProviders = state.providers.map((item) =>
    item.id === providerId
      ? {
          ...item,
          models: item.models.filter((entry) => entry !== model),
          updatedAt: now,
        }
      : item,
  );
  ...
  if (!nextProvider?.models.includes(nextActiveByProvider[providerId] || "")) {
